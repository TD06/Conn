...
import { CHECH_USER_ROUTE, GET_MESSAGES_ROUTE, HOST } from "@/utils/ApiRoutes";
...
function Main() {
  ...
  onAuthStateChanged(firebaseAuth, async(currentUser)=>{
    ...
    if(!userInfo && currentUser?.email){
      const {data} = await axios.post(CHECH_USER_ROUTE,{email:currentUser.email})
  ...
  return (
    ...
    <div className="grid grid-cols-main h-screen w-screen max-h-screen max-width-full overflow-hidden">
      ...
      {
        currentChatUser ?  (
        <div className={messagesSearch ? "grid grid-flow-col" : " grid-cols-2"}>
          <Chat />
